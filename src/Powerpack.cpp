...
Powerpack::Powerpack(ros::NodeHandle& nh,
    ...
    for (int i = 0; i < databaseconfig->get_n_pos_channel(); ++i) {
        auto tmp_solver = std::make_unique<Solver>(
            positive, sensor, referencegoverner, databaseconfig);
        solvers.push_back(std::move(tmp_solver));
    }
    for (int i = 0; i < databaseconfig->get_n_neg_channel(); ++i) {
        auto tmp_solver = std::make_unique<Solver>(
            negative, sensor, referencegoverner, databaseconfig);
        solvers.push_back(std::move(tmp_solver));
    }
    int channel = 0;
...
//     std::cout << "number of positive channel : " << n_pos_channel <<
//     std::endl; std::cout << "number of negative channel : " << n_neg_channel
//     << std::endl; std::cout << "--------------------" << std::endl; std::cout
//     << "controller type" << std::endl; for (const auto& tmp_module : modules)
//     {
...
//     std::cout << "number of sensor channel : " << sensor->get_n_channel() <<
//     std::endl; std::cout << "--------------------" << std::endl; std::cout <<
//     "PWM info" << std::endl; std::cout << "number of pwm channel : " <<
//     pwm->get_n_channel() << std::endl;
...
//         modules[i].calculate_control_signal(sensor->get_data(i+3),sensor->get_data(0),sensor->get_data(2),
//         referencegoverner->get_data(i));
//     }
//     for( int i = (0 + n_pos_channel); i < (n_pos_channel+n_neg_channel);++i)
//     {
//         modules[i].calculate_control_signal(sensor->get_data(i+3),sensor->get_data(1),sensor->get_data(2),
//         referencegoverner->get_data(i));
//     }
// }
...
//     for (const auto& tmp_module : modules) {
//         std::vector<unsigned int> tmp_value =
//         tmp_module.get_control_signal(); pwm_signal.insert(pwm_signal.end(),
//         tmp_value.begin(), tmp_value.end());
//     }
